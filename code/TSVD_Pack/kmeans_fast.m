function [ centers, mincenter, mindist, q2, quality ] = kmeans_fast ( ...
  data, initcenters, method, calc_final_dist )

%*****************************************************************************
%
%% KMEANS_FAST carries out the fast KMEANS algorithm of Charles Elkan.
%
%  !! Code modified slightly to work with dim*n data (for efficiency with sparse matrix) !!
%  
%  Discussion:
%
...
%  Parameters:
%
%    Input, real DATA(M,N), the M-dimensional coordinates of N points.
%
...
%    
%    Input, integer CALC_FINAL_DIST
%    * 1, calculate the final distance of each point to closest center
%
...
%   
    ...
    if size(diff,1) < n/3 && iteration > 0
         for j = diffj
            ...
            centers(j,:) = (centers(j,:)*oldpop + sum(data(:,diff(plus)),2)' - sum(data(:,diff(minus)),2)')/pop(j); 
        end
    else
        for j = diffj
...
% it's correct to have mean(data(track,:),1) but this can make answer worse!
            centers(j,:) = mean(data(:,track),2)';
        end
    end
    
    if method == 2
        for j = diffj
            offset = calcdist2(centers(j,:),oldcenters(j,:));
            computed = computed + 1;
...
if calc_final_dist == 1
  if n*dim <= 5e7 || ~issparse(data)  % !!CALCULATING ALL DISTANCES ONLY FOR SMALL DATASETS
...
%       fprintf ( 1, '  %4d  %g  %g  %g  %d\n', iteration, toc, quality, q2, total );
      ...
      q2=0;
      csize = dim/2;
      chunks = floor(n/csize) + (mod(n,csize)>0);
      for c =1:chunks
            udist = calcdist(data(:,(c-1)*csize+1:min(c*csize,n)), ...
                centers(mincenter((c-1)*csize+1:min(c*csize,n)),:));
            quality = quality + sum(udist);
            q2 = q2 + sum(udist.^2);
      end
      quality = quality/n;
      q2 = q2/n;
